...
if hparams.model_hparams.share_embedding:
    shared_vocab_filepath = os.path.join(model_dir, Vocabulary.SHARED_VOCAB_FILENAME)
    input_vocabulary = Vocabulary.load(shared_vocab_filepath)
    output_vocabulary = input_vocabulary
else:
    input_vocab_filepath = os.path.join(model_dir, Vocabulary.INPUT_VOCAB_FILENAME)
    input_vocabulary = Vocabulary.load(input_vocab_filepath)
    output_vocab_filepath = os.path.join(model_dir, Vocabulary.OUTPUT_VOCAB_FILENAME)
...
with ChatbotModel(mode = "infer",
    ...
    # Setting up the chat
    chatlog_filepath = os.path.join(model_dir, "chat_logs", "chatlog_{0}.txt".format(datetime.datetime.now().strftime("%Y%m%d_%H%M%S")))
