...
class ChatbotModel(object):
    ...
    def train_batch(self, inputs, targets, input_sequence_length, target_sequence_length, learning_rate, dropout, global_step, log_summary=True):
        ...
        Args:
            ...
            global_step: The index of this training step across all batches and all epochs.

            log_summary: Flag indicating if the training summary should be logged.
        """
    ...
    def predict_batch(self, inputs, input_sequence_length, max_output_sequence_length, beam_length_penalty_weight, sampling_temperature, log_summary=True):
        ...
        Args:
            ...
            log_summary: Flag indicating if the inference summary should be logged.
        """
    ...
    def chat(self, question, chat_settings):
        ...
        'question' and 'answer' in this context are generic terms for the interactions
        ...
        if self.beam_width > 0:
            #For beam search decoding: if show_all_beams is enabled then output all beams (sequences), otherwise take the first beam.
    ...
    def _create_rnn_cell(self, rnn_size, num_layers, keep_prob):
        ...
        for _ in range(num_layers):
            if self.model_hparams.rnn_cell_type == "lstm":
                rnn_cell = tf.contrib.rnn.BasicLSTMCell(num_units=rnn_size)
