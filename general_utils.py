...
def initialize_session(mode):
    ...
    if mode == "train":
        ...
        ex_group.add_argument("--datasetdir", "-d", help="Path structured as datasets/dataset_name. A new model will be trained using the dataset_old contained in this directory.")
    ...
    if mode == "train" and args.datasetdir:
        #Make sure dataset_old exists
        dataset_dir = os.path.relpath(args.datasetdir)
        if not os.path.isdir(dataset_dir):
            raise NotADirectoryError("Cannot find dataset_old directory '{0}'".format(os.path.realpath(dataset_dir)))
